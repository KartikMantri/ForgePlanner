...
import { Plus, ArrowRight, Target, Flame, Code2, BookOpen, LogOut, HelpCircle, KeyRound, X } from 'lucide-react';
import IronManStrike from './components/ironman/IronManStrike';
import './index.css';
...
const MasterDashboard = () => {
  ...
  const [strikeTarget, setStrikeTarget] = useState<{ goal: any; rect: DOMRect } | null>(null);
  const [burstingId, setBurstingId] = useState<string | null>(null);
  const cardRefs = useRef<Record<string, HTMLDivElement | null>>({});
  const deletePromiseRef = useRef<Promise<any> | null>(null);
  const navigate = useNavigate();

  const handleConfirmDelete = async () => {
    if (!confirmGoal) return;
    const goal = confirmGoal;
    const rect = cardRefs.current[goal.id]?.getBoundingClientRect();
    setConfirmGoal(null);

    if (!rect) {
      // Fallback: no card ref to animate against, just delete outright
      try {
    ...
    deletePromiseRef.current = goalsApi.deleteGoal(goal.id).catch((e) => {
      console.error(e);
      return null;
    });
    setStrikeTarget({ goal, rect });
  };

  const handleStrikeImpact = () => {
    if (strikeTarget) setBurstingId(strikeTarget.goal.id);
  };

  const handleStrikeDone = async () => {
    await deletePromiseRef.current;
    const goalId = strikeTarget?.goal.id;
    ...
    <div className="min-h-screen bg-background text-foreground relative flex flex-col font-military selection:bg-[var(--color-arc-cyan)] selection:text-black">
        ...
        <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 pb-20">
          {goals.map((goal: any, index: number) => (
            <div
              ...
              onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') navigate(`/goals/${goal.id}`); }}
              onMouseMove={(e) => handleMouseMove(e, index)}
              onMouseLeave={(e) => handleMouseLeave(e)}
              className={`iron-panel text-left p-4 sm:p-6 transition-all duration-200 group active:scale-95 touch:scale-100 cursor-pointer ${burstingId === goal.id ? 'animate-goal-burst' : ''}`}
            ...
            >
              ...
              <button
                ...
                title="Delete goal"
                className="absolute top-2 right-2 sm:top-3 sm:right-3 z-20 p-1.5 rounded-full bg-black/60 border border-white/10 text-white/50 hover:text-red-400 hover:border-red-400/50 transition-colors"
              >
      ...
      {confirmGoal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-background border border-[var(--color-arc-cyan)]/30 rounded-2xl p-6 max-w-sm w-full text-center shadow-2xl">
            <h3 className="font-display font-bold text-lg text-white mb-2 tracking-wide">DELETE GOAL?</h3>
            <p className="text-white/60 text-sm mb-6">
              Are you sure you want to delete "<span className="text-white font-medium">{confirmGoal.title}</span>"? This can't be undone.
            </p>
            <div className="flex gap-3">
              <button
                onClick={() => setConfirmGoal(null)}
                className="flex-1 px-4 py-2.5 border border-white/20 text-white/70 rounded-lg hover:bg-white/10 transition-colors font-display tracking-wide text-sm"
              >
                NO
              </button>
              <button
                onClick={handleConfirmDelete}
                className="flex-1 px-4 py-2.5 bg-red-500/20 border border-red-500 text-red-400 rounded-lg hover:bg-red-500 hover:text-white transition-colors font-display font-bold tracking-wide text-sm"
              >
                YES
              </button>
            </div>
      ...
      {/* Iron Man strike — flies in and shoots the card being deleted */}
      {strikeTarget && (
        <IronManStrike
          targetRect={strikeTarget.rect}
          onImpact={handleStrikeImpact}
          onDone={handleStrikeDone}
        />
      )}
    </div>
  );
